...
namespace Thek_GuardingPawns
{
    public class Window_SortPatrolSpots(SortedList<int, Thing> listForDef) : Window
    {
        ...
        private void DoListEntries(Rect Rect)
        {
            ...
            {
                Rect UpButtonRect = new(scrollView.Rect.xMax - (UpButtonWidth + 5f + padding) * 2,
                                        listing_Standard.CurHeight + 3f,
                                        UpButtonWidth,
                                        UpButtonHeight);


                Rect DownButtonRect = new(UpButtonRect.xMax + 1f + padding,
                                          listing_Standard.CurHeight + 3f,
                                          UpButtonRect.width,
                                          UpButtonRect.height);


                Rect highlightRect = new(scrollView.Rect.xMin,
                                         listing_Standard.CurHeight,
                                         Rect.width,
                                        UpButtonHeight);


                Rect jumpRect = new(scrollView.Rect.xMin,
                                    listing_Standard.CurHeight,
                                    Rect.width - UpButtonRect.width - DownButtonRect.width - (padding * 3) - 8f,
                                    UpButtonHeight);
